...
def write_summary( summary_file, mqfile, ncol, input):#, names):
    """
    ...
    a, b = input_to_dict(input)#, names)
    data = diagram(mqfile, ncol, a, b)
...
def input_to_dict(inputs):#comp_files, names):
    ...
    c = ["B", "C", "D", "E", "F"]  # "A" is for MQ file
    for i in range(len(inputs)):#comp_files)):
        if inputs[i][2] == "file":
	    file = inputs[i][0]#comp_files[i]
	    #print(file)
            name = inputs[i][1].replace(".txt", "")#names[i].replace(".txt", "") #file.split("/")[-1].replace(".txt", "")
	    title = c[i]
...
def diagram(MQfile, ncol, comp_dict, title_dict):
...
def process( args ):
        ...
        write_summary( args.summary, args.mq, args.ncol, args.input)#, args.names)
...
if __name__ == '__main__':
    ...
    group_input.add_argument('--mq', nargs=3, required=True)
