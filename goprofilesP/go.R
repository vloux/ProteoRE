...
#str(test)
#str(test$Gene.names)
getprofile = function(filename, ncol, level) {
...
goooo = function() {
  ...
  if (length(args) != 7) {
    ...
    profiles = getprofile(input, ncol, level)
...
#Rscript go.R ../filter_kw_val/proteinGroups_Maud.txt "7" "CC" "PDF" 2 "TRUE" "Title"
