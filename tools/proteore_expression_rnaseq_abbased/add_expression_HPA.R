...
check_ensembl_geneids <- function(vector, type) {
  ensembl_geneid_pattern <- "^ENS[A-Z]+[0-9]{11}$|
  ^[A-Z]{3}[0-9]{3}[A-Za-z](-[A-Za-z])?$|^CG[0-9]+$|
  ^[A-Z0-9]+[.][0-9]+$|^YM[A-Z][0-9]{3}[a-z][0-9]$"
  res <- grepl(ensembl_geneid_pattern, vector)
  if (all(!res)) {
    cat("No Ensembl geneIDs found in entered ids")
    stopquietly()
  } else if (any(!res)) {
    cat(paste(sep = "", collapse = " ", c(sum(!res, na.rm = TRUE),
                                          "IDs are not ENSG IDs,
                                          please check:\n")))
    not_geneids <- sapply(vector[which(!res)], function(x) paste(
      sep = "", collapse = "", x, "\n"), USE.NAMES = F)
    cat(not_geneids)
  }
}
  ...
  if (length(unlist(strsplit(as.character(line[ncol]), ";"))) > 1) {
    if (length(line) == 1) {
      ...
      if (ncol == 1) {                #first column
        ...
        lines <- suppressWarnings(cbind(line[1:ncol - 1],
                  unlist(strsplit(as.character(line[ncol]), ";"),
                         use.names = F), line[(ncol + 1):length(line)]))
      }
    }
...
get_args <- function() {
  ...
  if ("--help" %in% args) {
    ...
    Arguments:
        ...
        --input: either a file name (e.g : input.txt) or a list of
        blank-separated ENSG identifiers (e.g : ENSG00000283071 ENSG00000283072)
...
is_col_in_file <- function(file, ncol) {
  ...
  if (!is_in_file) {
    cat(paste(sep = "", collapse = " ",
              c("Column", ncol, "not found in file")))
    stopquietly()
  }
}

convert_to_previous_header <- function(options) {
    header <- c("Gene", "Gene description", "Evidence", "Antibody",
               "RNA tissue specificity", "Reliability (IH)", "Reliability (IF)",
               "Subcellular location", "RNA tissue specific NX",
               "TPM max in non-specific")
    names(header) <- c("Gene", "Gene description", "Evidence", "Antibody",
                       "RNA tissue category", "Reliability (IH)",
                       "Reliability (IF)", "Subcellular location",
                       "RNA TS TPM", "TPM max in non-specific")
...
main <- function() {
  ...
  if (inputtype == "copypaste") {
    ...
    ids <- unlist(sapply(as.character(file[, ncol]),
  ...
  if (tail(unlist(strsplit(args$atlas, "/")), 1) ==
      "HPA_full_atlas_23-10-2018.tsv") {
  ...
  # Write output
  if (is.null(res)) {
    write.table("None of the ENSG ids entered can be found in HPA data file",
  file = output, sep = "\t", quote = FALSE, col.names = TRUE, row.names = FALSE)
  } else {
    if (inputtype == "copypaste") {
      ids <- data.frame(ids)
      output_content <- merge(ids, res, by.x = 1, by.y = "row.names",
                        incomparables = NA, all.x = T)
      colnames(output_content)[1] <- "Ensembl"
    } else if (inputtype == "tabfile") {
      output_content <- merge(file, res, by.x = ncol, by.y = "row.names",
                             incomparables = NA, all.x = T)
