...
fortify_compareclusterresult <- function(res_cmp, showCategory = 30,
  ...
  if (is.null(showCategory)) {
    ...
    if (!is.null(.split) && .split %in% colnames(clprof_df)) {
      lres <- split(clprof_df, as.character(clprof_df[, .split]))
      lres <- lapply(lres, topN, showCategory = showCategory)
      result <- do.call("rbind", lres)
    } else {
      result <- topN(clprof_df, showCategory)
    }
  }
  ID <- NULL
  if (includeAll == TRUE) {
    result <- subset(clprof_df, ID %in% result$ID)
  }
  ## remove zero count
...
plotting.clusterProfile <- function(clProf.reshape.df, x = ~Cluster,
                type = "dot", colorBy = "p.adjust", by = "geneRatio",
                title="", font.size=12) {
...
make_dotplot <- function(res_cmp, ontology) {
  ...
  dfok$Description <- sapply(as.vector(dfok$Description), function(x) {
...
main <- function() {
  ...
  for (ontology in ont) {
    ...
    res_cmp <- cmp.GO(l = l, fun = "enrichGO", orgdb, ontology, readable = TRUE)
    make_dotplot(res_cmp, ontology)
    output_path <- paste("GO_enrich_comp_", ontology, ".tsv", sep = "")
    write.table(res_cmp@compareClusterResult, output_path, sep = "\t",
                row.names = F, quote = F)
  }
