...
split_ids_per_line <- function(line, ncol) {
  ...
  if (length(unlist(strsplit(as.character(line[ncol]), ";"))) > 1) {
    if (length(line) == 1) {
      lines <- as.data.frame(unlist(
        strsplit(as.character(line[ncol]), ";")), stringsAsFactors = F)
    } else {
...
#create new lines if there's more than one id per cell in the column in order
...
main <- function() {
  ...
  if (input_type == "list") {
    ...
    input <- sapply(file[, ncol], function(x) strsplit(
      as.character(x), ";")[[1]][1], USE.NAMES = F)
  }
  ...
  df <- as.data.frame(t(as.data.frame(tmp)), row.names = c(""),
                      stringsAsFactors = FALSE)
  names(df) <- c("date", "tissue", "filename", "path")

  # Annotations
  res <- sapply(df$path, function(x) nb_obs_peptideatlas(input, x),
                USE.NAMES = FALSE)
