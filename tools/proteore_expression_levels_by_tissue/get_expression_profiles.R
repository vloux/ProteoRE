...
annot_HPAnorm <- function(input, HPA_normal_tissue, tissue, level, reliability, not_mapped_option) {
...
annot_HPAcancer <- function(input, HPA_cancer_tissue, cancer, not_mapped_option) {
...
main <- function() {
  ...
  if (atlas == "normal") {
    ...
    # Calculation
    res <- annot_HPAnorm(ids, reference_file, tissue, level, reliability, not_mapped_option)
