...
def get_args(nb_col_to_int):
    parser = argparse.ArgumentParser()
    parser.add_argument("--species")
    parser.add_argument("--database", help="Humap, Bioplex or Biogrid", required=True)  # noqa E501
    parser.add_argument("--dict_path", required=True)
    parser.add_argument("--input_type", help="type of input (list of id or filename)",required=True)  # noqa E501
...
def nb_col_to_int(nb_col, sys):
    try:
        nb_col = int(nb_col.replace("c", "")) - 1
        return nb_col
    except():
        sys.exit("Please specify the column where you would like to apply the filter with valid format")  # noqa E501
...
def blank_to_NA(csv_file):
    ...
    for line in csv_file:
        line = ["NA" if cell == "" or cell == " " or cell == "NaN" or cell == "-" else cell for cell in line]  # noqa E501
        tmp.append(line)
...
def biogrid_output_files(ids, species):
    network_file = [["Entrez Gene Interactor A",
                     "Entrez Gene Interactor B",
                     "Gene symbol Interactor A",
                     "Gene symbol Interactor B",
                     "Experimental System",
                     "Experimental Type",
                     "Pubmed ID",
                     "Interaction Score",
    ...
    for id in ids:
        if id in ppi_dict['network']:
            ...
            ids_set.update([interact[1] for interact in ppi_dict['network'][id]])  # noqa E501
        else:
            ids_not_found.add(id)

    nodes_file = [["Entrez gene ID",
                   "Official Symbol Interactor",
                   "Present in user input ids",
                   "ID present in Biogrid "+species,
                   "Pathway"]]
    for id in ids_set:
        # get pathway
        ...
        # get gene name
        if id in ppi_dict['network']:
            gene_name = ppi_dict['network'][id][0][2]
        else:
            gene_name = "NA"

        # make line
        nodes_file.append([id]+[gene_name]+[id in ids]+[id not in ids_not_found]+[description_pathway])  # noqa E501
    return network_file, nodes_file


def bioplex_output_files(ids, id_type, species):
    network_file = [[id_type+" Interactor A",
                     id_type+" Interactor B",
                     "Gene symbol Interactor A",
                     "Gene symbol Interactor B",
                     "Interaction Score"]]
    ids_set = set(ids)
    ...
    for id in ids:
        if id in ppi_dict['network'][id_type]:
            ...
            ids_set.update([interact[1] for interact in ppi_dict['network'][id_type][id]])  # noqa E501
        else:
            ids_not_found.add(id)
    ...
    if id_type == "UniProt-AC":
        nodes_file = [[id_type,
                       "Present in user input ids",
                       "ID present in Human Bioplex",
                       "Pathway"]]
    else:
        nodes_file = [[id_type,
                       "Official symbol Interactor",
                       "Present in user input ids",
                       "Present in interactome",
                       "Pathway"]]
    for id in ids_set:
        ...
        # make line
        if id_type == "UniProt-AC":
            nodes_file.append([id]+[id in ids]+[id not in ids_not_found]+[description_pathway])  # noqa E501
        elif id_type == "GeneID":
            # get gene_name
            if id in ppi_dict['network'][id_type]:
                gene_name = ppi_dict['network'][id_type][id][0][2]
            else:
                gene_name = "NA"
            nodes_file.append([id]+[gene_name]+[id in ids]+[id not in ids_not_found]+[description_pathway])  # noqa E501
...
def humap_output_files(ids, species):
    ...
    for id in ids_set:
        ...
        # make line
        nodes_file.append([id]+[gene_name]+[id in ids]+[id not in ids_not_found]+[protein_name]+[description_pathway])   # noqa E501
...
def sort_by_column(tab, sort_col, reverse, header):
    ...
    if len(tab) > 1:  # if there's more than just a header or 1 row
        ...
        unsortable_lines = [i for i,line in enumerate(tab) if (line[sort_col]=='' or line[sort_col] == 'NA')]  # noqa E501
        ...
        if only_number(tab, sort_col) and any_float(tab, sort_col):
            tab = sorted(tab, key=lambda row: float(row[sort_col]), reverse=reverse)  # noqa E501
        elif only_number(tab, sort_col):
            tab = sorted(tab, key=lambda row: int(row[sort_col]), reverse=reverse)    # noqa E501  
        else:
...
def only_number(tab, col):
    ...
    for line in tab:
        if not (is_number("float", line[col].replace(",", ".")) or is_number("int", line[col].replace(",", "."))):  # noqa E501
            return False
    return True
...
def main():
    ...
    if args.input_type == "text":
        ...
        input_file, ids = get_input_ids_from_file(args.input, args.ncol,
                                                  args.header)
    ...
    with open(args.nodes_output, "w") as output:
        ...
        for row in nodes_file:
            writer.writerow([unicode(s).encode("utf-8") for s in row])  # noqa F821
